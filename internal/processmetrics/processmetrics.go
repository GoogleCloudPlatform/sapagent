...
const (
	...
	metricOverridePath             = "/etc/google-cloud-sap-agent/metrics-override.yaml"
...
func instancesWithCredentials(ctx context.Context, params *Parameters) *sapb.SAPInstances {
	...
	for _, instance := range sapInstances.GetInstances() {
		if instance.GetType() == sapb.InstanceType_HANA {
			...
			instance.HanaDbUser, instance.HanaDbPassword, instance.HdbuserstoreKey, err = sapdiscovery.ReadHANACredentials(ctx, projectID, hanaConfig, params.GCEService)
			if err != nil {
...
func startReliabilityMetrics(ctx context.Context, parameters Parameters) bool {
	...
	p := createReliabilityCollectors(ctx, parameters, sapInstances)
...
func (p *Properties) collectAndSendReliabilityMetrics(ctx context.Context, bo *cloudmonitoring.BackOffIntervals) error {
	...
	reliabilityCollectTicker := time.NewTicker(time.Duration(minimumFrequencyForReliability) * time.Second)
...
func (p *Properties) collectAndSendReliabilityMetricsOnce(ctx context.Context, bo *cloudmonitoring.BackOffIntervals) {
	var wg sync.WaitGroup
		...
		r := &recovery.RecoverableRoutine{
			Routine: func(ctx context.Context, a any) {
				...
				if args, ok := a.(collectReliabilityMetricsRoutineArgs); ok {
					if _, err := args.c.CollectWithRetry(ctx); err != nil {
...
func createDemoCollectors(ctx context.Context, params Parameters, client cloudmonitoring.TimeSeriesCreator, demoMetricsReader metricoverrides.DemoMetricsReaderFunc) *Properties {
	p := &Properties{
		Config:        params.Config,
		Client:        client,
		HeartbeatSpec: params.HeartbeatSpec,
	}
	p.Collectors = append(p.Collectors, &metricoverrides.DemoInstanceProperties{
		Config:           params.Config,
		DemoMetricPath:   metricOverridePath,
		Reader:           demoMetricsReader,
		MetricTypePrefix: pmMetricTypePrefix,
	})
	return p
}
