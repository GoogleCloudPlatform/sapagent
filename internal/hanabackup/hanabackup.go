...
func ParseBasePath(ctx context.Context, pattern string, exec commandlineexecutor.Execute) (string, error) {
	...
	if result.Error != nil || result.StdErr != "" {
		// sudo google_cloud_sap_agent hanadiskbackup -group-snapshot-name=nc-root-isg-test -host=localhost -port=30013 -sid=HDB -hana-db-user=system -password=Google123 -loglevel debug
