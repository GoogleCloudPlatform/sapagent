...
type gceInterface interface {
	...
	GetURIForIP(project, region, subnetwok, ip string) (string, error)
...
func (d *CloudDiscovery) discoverResource(ctx context.Context, host toDiscover, project string) (*spb.SapDiscovery_Resource, []toDiscover, error) {
	...
	if len(addrs) > 0 {
		...
		if c, ok := d.resourceCache[addr]; ok {
			...
			log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "addr", addr, "res", c.res, "toAdd", c.related)
		...
		if host.network != "" {
			...
			for _, s := range cloudNet.subnets {
				...
				if s.ipRange.Contains(ip) {
					...
					uri, err = d.GceService.GetURIForIP(project, host.region, s.name, addr)
					if err != nil {
		...
		if uri == "" {
			uri, err = d.GceService.GetURIForIP(project, host.region, "", addr)
		}
		if err != nil {
		...
		if c, ok := d.resourceCache[uri]; ok {
			...
			log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "uri", uri, "res", c.res, "toAdd", c.related)
			d.resourceCache[host.name] = c
			log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "addr", addr, "res", c.res, "toAdd", c.related)
	...
	log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "uri", uri, "res", res, "toAdd", toAdd)
	d.resourceCache[host.name] = c
	log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "uri", res.ResourceUri, "res", res, "toAdd", toAdd)
	...
	if addr != "" && addr != host.name {
		log.CtxLogger(ctx).Debugw("discoverResource adding to cache", "addr", addr, "res", res, "toAdd", toAdd)
		d.resourceCache[addr] = c
	}
