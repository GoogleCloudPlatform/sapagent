	...
	for _, s := range sapDetails {
		...
		if s.DBComponent != nil {
			...
			if s.DBInstance != nil && s.DBInstance.HanaReplicationTree != nil {
				...
				primarySite.Component.Sid = s.DBComponent.GetSid()
				s.DBComponent = primarySite.Component

				// Find the site this instance belongs to.
				hostIsPrimary := false
				for _, r := range s.DBComponent.Resources {
					if r.GetResourceUri() == instanceResource.GetResourceUri() {
						// Add the host resources to this site.
						log.CtxLogger(ctx).Debugw("Adding host resources to primary site", "resources", dbRes)
						s.DBComponent.Resources = removeDuplicates(append(s.DBComponent.Resources, dbRes...))
						hostIsPrimary = true
						break
					}
				}
				if !hostIsPrimary {
					for _, site := range s.DBComponent.ReplicationSites {
						for _, r := range site.Component.Resources {
							if r.GetResourceUri() == instanceResource.GetResourceUri() {
								log.CtxLogger(ctx).Debugw("Adding host resources to secondary site", "resources", dbRes, "site", site)
								// Add the host resources to this site.
								site.Component.Resources = removeDuplicates(append(site.Component.Resources, dbRes...))
								break
							}
						}
					}
...
func resArrayToString(r []*spb.SapDiscovery_Resource) string {
	var res []string
	for _, r := range r {
		res = append(res, prototext.Format(r))
	}
...
func (d *Discovery) discoverReplicationSite(ctx context.Context, site *sappb.HANAReplicaSite, sid string, instanceResource *spb.SapDiscovery_Resource, instanceSubnetwork string, lbGroups []loadBalancerGroup, cp *ipb.CloudProperties) *spb.SapDiscovery_Component_ReplicationSite {
	...
	log.CtxLogger(ctx).Debugw("Site Resources", "site", site.Name, "resources", resArrayToString(siteRes))
	...
	for _, r := range repComp.Resources {
		if r.GetResourceKind() == spb.SapDiscovery_Resource_RESOURCE_KIND_INSTANCE && r.GetInstanceProperties().GetVirtualHostname() == site.Name {
