...
func (a *AiAnalyzer) fetchNameserverTraces(ctx context.Context) (string, error) {
	...
	if err != nil {
		a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while reading support bundle directory", err)
		return "", err
	}
	...
	for _, file := range files {
		...
		if filterNameserverTraces(ctx, file.Name()) {
			...
			if err != nil {
				a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while reading support bundle file", err)
				return "", err
			}

			filteredContent, err := a.extractTimeWindowFromText(ctx, string(content), false)
			if err != nil {
				a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while extracting nameserver traces within time window from text", err)
				return "", err
...
func (a *AiAnalyzer) downloadLogparser(ctx context.Context, httpGet httpGet) error {
	...
	if err != nil {
		a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while creating logparser file", err)
		return err
	}
	defer outFile.Close()
	...
	if err != nil {
		a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while getting logparser file", err)
		return err
	}
	...
	if err != nil {
		a.oteLogger.LogErrorToFileAndConsole(ctx, "Error while writing logparser file", err)
		return err
	}
