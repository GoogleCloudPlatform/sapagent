...
func (s *SupportBundle) supportBundleHandler(ctx context.Context, destFilePathPrefix string, exec commandlineexecutor.Execute, fs filesystem.FileSystem, z zipper.Zipper, cp *ipb.CloudProperties) (string, subcommands.ExitStatus) {
	...
	if s.ProcessMetrics {
		metrics := []string{"sap/hana/volumes", "sap/control/memory/utilization", "sap/control/cpu/utilization"}
		if errMsgs := s.collectProcessMetrics(ctx, metrics, "/tmp/google-cloud-sap-agent", cp, fs); len(errMsgs) > 0 {
...
func (s *SupportBundle) collectProcessMetrics(ctx context.Context, metrics []string, destFilesPath string, cp *ipb.CloudProperties, fs filesystem.FileSystem) []string {
	...
	for _, metric := range metrics {
		...
		if err != nil {
			log.CtxLogger(ctx).Errorw(fmt.Sprintf("Failed to fetch time series data for metric %s", metric), "error", err)
...
func (s *SupportBundle) fetchTimeSeriesData(ctx context.Context, cp *ipb.CloudProperties, cmr *cloudmetricreader.CloudMetricReader, mmc cloudmonitoring.TimeSeriesDescriptorQuerier, metric string) ([]TimeSeries, error) {
	...
	if err != nil {
		log.CtxLogger(ctx).Errorw(fmt.Sprintf("Failed to fetch label descriptors for metric %s", metric), "error", err)
		return nil, err
	}

	// TODO: Update the query to use time interval.
	query := fmt.Sprintf("fetch gce_instance | metric '%s' | filter (resource.instance_id = '%s') | within 5m", metric, "9097952311994808831")
	...
	if err != nil {
		log.CtxLogger(ctx).Errorw("The QueryTimeSeries request for metrics has failed", "error", err)
		return nil, err
	}
...
func getLabelValue(ctx context.Context, lv *mrpb.LabelValue) (string, error) {
	...
	default:
		log.CtxLogger(ctx).Errorw("Unsupported label value type", "type", lv.Value)
...
func getPointValue(ctx context.Context, v *cpb.TypedValue) (string, error) {
	...
	default:
		log.CtxLogger(ctx).Errorw("Unsupported value type", "type", tv)
...
func (s *SupportBundle) fetchLabelDescriptors(ctx context.Context, cp *ipb.CloudProperties, mmc cloudmonitoring.TimeSeriesDescriptorQuerier, metricType, resourceType string) ([]string, error) {
	...
	if err != nil {
		log.CtxLogger(ctx).Errorw("Error while getting monitored resource descriptor", "err", err)
		return nil, err
	}
	...
	if err != nil {
		log.CtxLogger(ctx).Errorw("Error while getting metric descriptor", "err", err)
		return nil, err
	}
