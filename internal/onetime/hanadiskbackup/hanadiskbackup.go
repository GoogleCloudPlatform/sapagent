...
//	If the topology is scaleup,
//		* it checks if the disk(s) are provided or not. If not, it reads the disk mapping.
//		* if multiple disks are provided, it checks if they are equal to autodiscovered disks and that they belong to the same consistency group.
func (s *Snapshot) validateDisks(ctx context.Context, cp *ipb.CloudProperties, exec commandlineexecutor.Execute) (string, subcommands.ExitStatus) {
	...
	if isScaleout {
		...
		if s.Disk == "" {
			...
			if len(s.disks) > 1 {
				if s.Disks != "" {
					log.CtxLogger(ctx).Infow("Validating disks provided by the user", "-source-disks", s.Disks, "autodiscovered disks", s.disks)
					disksCheck := make(map[string]bool)
					for _, disk := range s.disks {
						disksCheck[disk] = true
					}

					incorrectDisks := []string{}
					for _, disk := range strings.Split(s.Disks, ",") {
						if _, ok := disksCheck[disk]; !ok {
							incorrectDisks = append(incorrectDisks, disk)
							continue
						}
						delete(disksCheck, disk)
					}
					if len(incorrectDisks) > 0 {
						errMessage := "ERROR: -source-disks contains disk(s) that do not backup /hana/data"
						s.oteLogger.LogErrorToFileAndConsole(ctx, errMessage, fmt.Errorf("disk(s): [%s] do not back up /hana/data", strings.Join(incorrectDisks, ",")))
						return errMessage, subcommands.ExitFailure
					}

					if len(disksCheck) > 0 {
						leftOutDisks := []string{}
						for disk := range disksCheck {
							leftOutDisks = append(leftOutDisks, disk)
						}
						errMessage := "ERROR: -source-disks does not contain the entire list of disks backing up /hana/data"
						s.oteLogger.LogErrorToFileAndConsole(ctx, errMessage, fmt.Errorf("disk(s): [%s] not provided in the list of disks backing up /hana/data", strings.Join(leftOutDisks, ",")))
						return errMessage, subcommands.ExitFailure
					}
				}
