...
type (
	...
	fakeDashboardsAPICallerWithItems struct {
...
func (f *fakeDashboardsAPICallerWithItems) ListDashboards(context.Context, *dashboardpb.ListDashboardsRequest, ...gax.CallOption) *dashboard.DashboardIterator {
	return newFakeDashboardIterator(f.dashboards)
}

func (f *fakeDashboardsAPICallerWithItems) CreateDashboard(ctx context.Context, req *dashboardpb.CreateDashboardRequest, opts ...gax.CallOption) (*dashboardpb.Dashboard, error) {
	if f.err != nil {
		return nil, f.err
	}
	return req.Dashboard, nil
}

func newFakeDashboardIterator(dashboards []*dashboardpb.Dashboard) *dashboard.DashboardIterator {
	it := &dashboard.DashboardIterator{}

	// Set unexported it.items
	itemsPtr := (*[]*dashboardpb.Dashboard)(unsafe.Pointer(it))
	*itemsPtr = dashboards

	// Set unexported it.nextFunc at offset 32 (items=24, pageInfo=8, nextFunc=8)
	nextFuncPtr := (*func() error)(unsafe.Pointer(uintptr(unsafe.Pointer(it)) + 32))
	*nextFuncPtr = func() error {
		if len(*itemsPtr) == 0 {
...
func TestMigrateHMADashboards(t *testing.T) {
	tests := []struct {
		...
		{
			name:               "MissingTimeSeriesFilter",
			project:            "test-project",
			dashboardAPIClient: &fakeDashboardsAPICaller{},
			dashboards: []*dashboardpb.Dashboard{
				{
					DisplayName: "MissingTimeSeriesFilter",
					Layout: &dashboardpb.Dashboard_MosaicLayout{
						MosaicLayout: &dashboardpb.MosaicLayout{
							Tiles: []*dashboardpb.MosaicLayout_Tile{
								{
...
func TestSynopsisForMigrateHMADashboards(t *testing.T) {
...
func TestUsageForMigrateHMADashboards(t *testing.T) {
	want := `Usage: migratehmadashboards -project=<project-name> [-h] [-loglevel=<debug|info|warn|error>] [-log-path=<log-path>]\n`
	m := MigrateHMADashboards{}
	got := m.Usage()
	if got != want {
...
func TestMigrateHMADDashboardHandler(t *testing.T) {
	tests := []struct {
		name   string
		client dashboardsAPICaller
		want   subcommands.ExitStatus
	}{
		{
			name: "SuccessWithItems",
			client: &fakeDashboardsAPICallerWithItems{
				dashboards: readDashboards([]string{
					"testdata/test_dashboard_with_hma_metrics.json",
				}),
			},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := m.migrateHMADashboardHandler(context.Background(), tc.client)
			if got != tc.want {
				t.Errorf("migrateHMADashboardHandler() = %v, want %v", got, tc.want)
			}
		})
	}
}
