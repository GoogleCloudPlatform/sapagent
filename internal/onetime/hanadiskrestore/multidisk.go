...
// in the -source-disks parameter match to autodiscovered disks,
// and are attached to the instance or not.
func (r *Restorer) scaleupDisksAttachedToInstance(ctx context.Context, cp *ipb.CloudProperties) error {
	...
	for _, d := range r.disks {
		disksCheck[d.disk.DiskName] = true
	}
	...
	if len(incorrectDisks) > 0 {
		return fmt.Errorf("-source-disks contains disk(s): [%s] that do not back up /hana/data", strings.Join(incorrectDisks, ","))
	}
	...
	if len(disksCheck) > 0 {
		...
		return fmt.Errorf("-source-disks does not contain disks: [%s] that also back up /hana/data", strings.Join(leftOutDisks, ","))
	}

	for _, d := range r.disks {
		_, ok, err := r.gceService.DiskAttachedToInstance(r.Project, r.DataDiskZone, cp.GetInstanceName(), d.disk.GetDiskName())
		if err != nil {
			return fmt.Errorf("failed to verify if disk %v is attached to the instance", d.disk.GetDiskName())
		} else if !ok {
			return fmt.Errorf("the disk data-disk-name=%v is not attached to the instance", d.disk.GetDiskName())
		}
	}
