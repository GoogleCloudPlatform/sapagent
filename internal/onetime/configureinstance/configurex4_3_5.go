...
var (
	//go:embed google-x4_3_5.conf
	googleX4Conf3_5 []byte

	systemConf3_5       = []string{"DefaultTimeoutStartSec=300s", "DefaultTimeoutStopSec=300s", "DefaultTasksMax=infinity"}
	logindConf3_5       = []string{"UserTasksMax="}
	modprobeConf3_5     = []byte("blacklist idxd\nblacklist hpilo\nblacklist acpi_cpufreq\nblacklist qat_4xxx\nblacklist intel_qat\n")
	grubLinuxDefault3_5 = `GRUB_CMDLINE_LINUX_DEFAULT="tsc=nowatchdog add_efi_memmap udev.children-max=512 nmi_watchdog=0 watchdog_thresh=60 mce=2 console=ttyS0,115200 earlyprintk=ttyS0,115200 uv_nmi.action=kdump bau=0 pci=nobar transparent_hugepage=never numa_balancing=disable clocksource=tsc"`
)

// configureX4 checks and applies OS settings on X4.
// Returns true if a reboot is required.
func (c *ConfigureInstance) configureX43_5(ctx context.Context) (bool, error) {
	log.CtxLogger(ctx).Info("Continuing with general X4 configurations.")
	rebootSystemdSystem, err := c.checkAndRegenerateLines(ctx, "/etc/systemd/system.conf", systemConf3_5)
	if err != nil {
		return false, err
	}
	rebootSystemdLogin, err := c.removeLines(ctx, "/etc/systemd/logind.conf", logindConf3_5)
	if err != nil {
		return false, err
	}
	rebootModprobe, err := c.checkAndRegenerateFile(ctx, "/etc/modprobe.d/google-x4.conf", modprobeConf3_5)
	if err != nil {
		return false, err
	}
	...
	if c.HyperThreading == hyperThreadingOff {
		...
		grubLinuxDefault3_5 = strings.TrimSuffix(grubLinuxDefault3_5, `"`) + ` nosmt"`
	}
	rebootGrub, err := c.checkAndRegenerateLines(ctx, "/etc/default/grub", []string{grubLinuxDefault3_5})
	if err != nil {
		return false, err
	}
	...
	rebootSLES, err := c.configureX4SLES3_5(ctx)
	if err != nil {
...
func (c *ConfigureInstance) configureX4SLES3_5(ctx context.Context) (bool, error) {
	...
	if err := c.saptuneService3_5(ctx); err != nil {
		return false, err
	}
	sapTuneReapply, err := c.checkAndRegenerateFile(ctx, "/etc/saptune/extra/google-x4.conf", googleX4Conf3_5)
	if err != nil {
		return false, err
	}
	if c.saptuneSolutions3_5(ctx) {
		sapTuneReapply = true
	}
	if err := c.saptuneReapply3_5(ctx, sapTuneReapply); err != nil {
		return false, err
	}
...
func (c *ConfigureInstance) saptuneService3_5(ctx context.Context) error {
...
func (c *ConfigureInstance) saptuneSolutions3_5(ctx context.Context) bool {
...
func (c *ConfigureInstance) saptuneReapply3_5(ctx context.Context, sapTuneReapply bool) error {
	if !sapTuneReapply {
